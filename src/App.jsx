...
function App() {
  ...
  const generateJSONList = () => {
    setJsonData([]);
    const userids = tableData.userid.split('\n').map(val => val.trim()).filter(val => val);
    const usernames = tableData.username.split('\n').map(val => val.trim()).filter(val => val);
    ...
    <div className="flex justify-between gap-2 min-h-screen bg-slate-200">
      <div className="w-1/3 p-2 space-y-2">
        <div className="space-x-2">
          ...
          <input type="checkbox" name="isUsersTable" id="isUsersTable" checked={isUsersTable} onChange={() => setIsUsersTable(!isUsersTable)} className="size-4" />
        </div>
